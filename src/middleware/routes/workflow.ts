...
export function workflowRouter(config: StreamByConfig): Router {
  ...
  // Single-workflow endpoints — each project has exactly one Architecture workflow
  ...
  router.patch('/projects/:id/workflow', async (req: Request, res: Response) => {
    try {
      ...
      const existing = (project.workflows ?? [])[0];
      if (!existing) return res.status(404).json({ message: 'No workflow found for this project' });
      ...
      if (project.dbType === 'sql') {
        ...
        const updatedWorkflows = (project.workflows ?? []).map((w: any, i: number) => i === 0 ? updated : w);
        await SqlProject.update({ _id: projectId }, { workflows: updatedWorkflows } as any);
      } else {
        await Project.update(
          { _id: new ObjectId(projectId), 'workflows.id': existing.id },
          {
            $set: {
